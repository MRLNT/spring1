package id.fazzbca.library.exceptions;
...
public class CustomExceptionHandler {
    ...
    public ResponseEntity<?> handleIllegalArgEx(IllegalAccessException e){
        return ResponseHandler.responseError(400, e.getMessage(), null);
    }

    @ExceptionHandler(value = NoSuchElementException.class)
    public ResponseEntity<?> handleElementEx(NoSuchElementException e){
    ...
    public ResponseEntity<?> handleMethodArgNotValidEx(MethodArgumentNotValidException e){
        ...
        return ResponseHandler.responseError(e.getStatusCode().value(), "error valiudation", null);
    }
}